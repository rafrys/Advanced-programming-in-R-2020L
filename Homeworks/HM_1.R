...
condiitonal_summary <- function(df, sex){
  pastecs::stat.desc(df$Height[df$Woman == sex])[c(9, 8, 12, 13)]
  }
...
conditional_summary_general <- function(df,factor_variable){
  for (i in unique(eval(substitute(factor_variable), df))){
...
conditional_summary_general(iris,Species)
